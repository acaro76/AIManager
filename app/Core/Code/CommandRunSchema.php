...
/**
 * Code — Fase 6 / F6.8. Schema di `code_command_runs`: il lifecycle PERSISTENTE dei comandi locali,
 * ISOLATO dagli LLM e ADDITIVO (la 036 non tocca chat/patch/verifiche).
 *
 * SOLO METADATI (regola d'audit): scope, turno assistant, `command_id` monouso, `digest`,
 * `policy_version`, programma, un `display_summary` SANIFICATO (mai path assoluti, mai il pattern
 * grezzo, mai output), lo stato del lifecycle, exit/durata/byte/troncamento, il `process_group_id`
 * (per il recovery), l'`execution_id` e i timestamp. Nello schema NON esiste una colonna per l'argv
 * completo o l'output: il piano canonico vive nel CommandStore protetto, l'output solo in streaming.
 */
final class CommandRunSchema
{
    public const TABLE = 'code_command_runs';
    ...
    public const STATES = [
        ...
        'killed', 'rejected', 'expired', 'denied', 'error',
    ];
    ...
    {
        return <<<'SQL'
            CREATE TABLE code_command_runs (
                ...
                started_at TEXT,
                finished_at TEXT,
                CHECK (state IN ('pending', 'running', 'completed', 'failed', 'timed_out', 'killed', 'rejected', 'expired', 'denied', 'error')),
    ...
    {
        return [
            'CREATE INDEX idx_code_command_runs_scope ON code_command_runs (workspace_id, code_session_id)',
            'CREATE INDEX idx_code_command_runs_assistant ON code_command_runs (assistant_conversation_id)',
            'CREATE INDEX idx_code_command_runs_state ON code_command_runs (state)',
        ];
    }
    ...
    private static function verifyColumns(Database $db, string $table, array &$problems): void
    {
        $expected = [
            ...
            'duration_ms' => ['INTEGER', 0, 0, null],
            'output_bytes' => ['INTEGER', 0, 0, null],
            'truncated' => ['INTEGER', 1, 0, '0'],
            'process_group_id' => ['INTEGER', 0, 0, null],
    ...
    {
        $expected = [
            'idx_code_command_runs_scope' => ['workspace_id', 'code_session_id'],
            'idx_code_command_runs_assistant' => ['assistant_conversation_id'],
            'idx_code_command_runs_state' => ['state'],
        ];
        $present = [];
